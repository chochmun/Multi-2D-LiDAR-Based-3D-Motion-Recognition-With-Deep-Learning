...
class MainApp(QtWidgets.QMainWindow):
    ...
    def load_key_mapping(self):
        return {
            ...
            4: [None,None],#만세 - 상호작용1
            5: ['m',None],#스쿼트 -상호작용2
            6: ['f',None],#플랭크 - 상호작용3 
            7: ['p',None],#너무 가까움 -Pause
            1:['p',None]#사람없음 -Pause
        }, {
            #0: Stand motion -> None presspw
            0: ['w',None],#차렷자세 -> 전진
            -1: ['s',None], #(미구현) 
            2: ['a','w'], #왼손 들기 -> 좌 이동
            3: ['d','w'],#오른손 들기 -> 우 이동
    ...
    def start_connect_unity_function(self):
        # 유니티 연동 함수
        self.async_handler = AsyncTaskHandler(self.multi_lidar_services, MainApp, self.mplcanvas, self.key_input_approval,self.selected_angle,self.key_mapping_play1,self.key_mapping_play2)
...
class AsyncTaskHandler:
    def __init__(self, lidar_service, MainApp, canvas, key_input_approval,selected_angle,key_mapping_play1,key_mapping_play2):
        self.main_app = MainApp
        self.lidar_service = lidar_service
        self.main_app.idx_best_motion1 = 0
        self.main_app.idx_best_motion2 = 0
        self.is_running = True
        self.main_app.key_input_approval = key_input_approval
        self.main_app.key_mapping_play1=key_mapping_play1
        self.main_app.key_mapping_play2=key_mapping_play2
        self.canvas = canvas
        self.angle= selected_angle
        self.delays = deque(maxlen=50)  # 최대 길이를 50으로 설정
        self.motion_predictions = deque(maxlen=50)
        self.filtered_data1 = None  # 가장 최근 LiDAR 데이터를 저장할 변수
        self.filtered_data2 = None  # 가장 최근 LiDAR 데이터를 저장할 변수
        self.delay_and_acc_queue = queue.Queue()  # 플롯 업데이트를 위한 큐
        self.keyboard = Controller()

        # LiDAR 데이터 수집, AI 모션 추론, Plot 업데이트 쓰레드
        self.lidar_thread = threading.Thread(target=self.lidar_data_thread)
        self.ai_thread = threading.Thread(target=self.ai_motion_inference_thread)
        self.plot_thread = threading.Thread(target=self.plot_update_thread)
        ...
        # 전체 쓰레드를 시작할 메인 쓰레드
        self.async_thread = threading.Thread(target=self.start_threads)

    def setup_main_plot(self):
        """메인 FuncAnimation 플롯 설정"""
        ...
        self.scatter1 = self.ax2.scatter([], [], color='orange', marker='s',label="player1")
        self.scatter2 = self.ax2.scatter([], [], color='green',alpha=0.5, label="player2")
        self.ax2.set_xlim(0, 50)
        self.ax2.set_yticks(range(len(self.labels)))
        self.ax2.set_yticklabels(self.labels)
        self.ax2.legend(loc="upper right")
        def update_main_plot(i):
            """메인 플롯을 업데이트"""
            self.line1.set_data(list(range(len(self.delays))), self.delays)
            #print('########==========',self.delays)
            if self.delays and self.motion_predictions:
                if self.angle > 91:
                    # angle이 91도를 넘을 때, predictions1과 predictions2를 사용
                    predictions1 = [pred[0] for pred in self.motion_predictions]
                    predictions2 = [pred[1] for pred in self.motion_predictions]
                    
                    # scatter1에는 predictions1을, scatter2에는 predictions2를 설정
                    self.scatter1.set_offsets(np.column_stack([list(range(len(predictions1))), predictions1]))
                    self.scatter2.set_offsets(np.column_stack([list(range(len(predictions2))), predictions2]))
                else:
                    # angle이 91도 이하일 때는 기존의 `motion_predictions`를 scatter1에 설정
                    self.scatter1.set_offsets(np.column_stack([list(range(len(self.motion_predictions))), self.motion_predictions]))
            
            return self.line1, self.scatter1, self.scatter2
        self.ani = FuncAnimation(self.fig, update_main_plot, blit=True, interval=50)
        plt.ion()
        plt.tight_layout()
        plt.show()
    ...
    def lidar_data_thread(self):
        while self.is_running:
            data = self.lidar_service.return_filtered_data()

            if self.angle > 91:  # angle is 180
                data1,data2 = data
                self.filtered_data1=[item for sublist in data1 for item in sublist]
                self.filtered_data2=[item for sublist in data2 for item in sublist]
            else:  # angle is 90
                self.filtered_data1 = [item for sublist in data for item in sublist]

    def ai_motion_inference_thread(self):
        while self.is_running:
            if self.filtered_data1 is not None:
                start_time = time.time()
                motion_accuracies1 = self.lidar_service.get_motion_by_AI(self.filtered_data1)
                if self.angle>91:
                    motion_accuracies2 = self.lidar_service.get_motion_by_AI(self.filtered_data2)
                else:
                    motion_accuracies2=None
                
                delay = time.time() - start_time
                self.delay_and_acc_queue.put((delay, motion_accuracies1,motion_accuracies2))
    


    def plot_update_thread(self):
        """개발용 실시간 모션 데이터 업데이트 플롯"""
        while self.is_running:
            if not self.delay_and_acc_queue.empty():
                delay, motion_accuracies1,motion_accuracies2 = self.delay_and_acc_queue.get()
                self.main_app.idx_best_motion1, self.main_app.idx_best_motion2 = \
                    self.canvas.update_plot_by_realtime_motion(motion_accuracies1, motion_accuracies2)
                
                # 메인 FuncAnimation 플롯용 데이터 업데이트
                if self.angle>91:
                    self.motion_predictions.append((self.main_app.idx_best_motion1,self.main_app.idx_best_motion2))
                    print(self.motion_predictions)
                else:
                    self.motion_predictions.append(self.main_app.idx_best_motion1)
                self.delays.append(delay)

                print("##########################",self.main_app.idx_best_motion1,self.main_app.idx_best_motion2)
                if self.main_app.key_input_approval:
                    #print('########')
                    threading.Thread(target=self.press_keyboard, args=(self.main_app.idx_best_motion1, self.main_app.key_mapping_play1, 0.05)).start()
                    threading.Thread(target=self.press_keyboard, args=(self.main_app.idx_best_motion2, self.main_app.key_mapping_play2, 0.05)).start()
            else:
                time.sleep(0.02)
    ...
    def press_keyboard(self, idx_best_motion, player_keys, keydown_time=0.05):
        keys=[None,None]
        print("!#$!#%!#",idx_best_motion,"###",player_keys)
