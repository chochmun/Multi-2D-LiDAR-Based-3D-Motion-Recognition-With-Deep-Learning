...
class MainApp(QtWidgets.QMainWindow):
    ...
    def start_connect_unity_function(self):
        # 유니티 연동 함수
        self.async_handler = AsyncTaskHandler(
            lidar_service=self.multi_lidar_services,
            MainApp=self,  # MainApp 클래스 대신 main_app 인스턴스를 전달
            canvas=self.mplcanvas,
            key_input_approval=self.key_input_approval,
            selected_angle=self.selected_angle,
            key_mapping_play1=self.key_mapping_play1,
            key_mapping_play2=self.key_mapping_play2
        )
...
class AsyncTaskHandler:
    def __init__(self, lidar_service, MainApp, canvas, key_input_approval,selected_angle,key_mapping_play1,key_mapping_play2):
        ...
        self.idx_best_motion1 = 0
        self.idx_best_motion2 = 0
        self.is_running = True
    ...
    def plot_update_thread(self):
        ...
        while self.is_running:
            if not self.delay_and_acc_queue.empty():
                ...
                self.idx_best_motion1, self.idx_best_motion2 = \
                    self.canvas.update_plot_by_realtime_motion(motion_accuracies1, motion_accuracies2)
                
                # 메인 FuncAnimation 플롯용 데이터 업데이트
                if self.angle>91:
                    self.motion_predictions.append((self.idx_best_motion1,self.idx_best_motion2))
                    print(self.motion_predictions)
                else:
                    self.motion_predictions.append(self.idx_best_motion1)
                self.delays.append(delay)

                print("##########################",self.idx_best_motion1,self.idx_best_motion2)
                if self.main_app.key_input_approval:
                    #print('########')
                    threading.Thread(target=self.press_keyboard, args=(self.idx_best_motion1, self.main_app.key_mapping_play1, 0.05)).start()
                    threading.Thread(target=self.press_keyboard, args=(self.idx_best_motion2, self.main_app.key_mapping_play2, 0.05)).start()
            else:
                time.sleep(0.02)
