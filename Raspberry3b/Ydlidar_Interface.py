...
D_end_angle = int (180+(90-D_ANGLE/2))

class YDLidarX2:
    
    def __init__(self, port, chunk_size=1000): #spot 탐지용은 청크를 1000내려도되는데, 그게 아닌이상 1000이상 유지
        self.__version = 1.03
        ...
        self._max_range = 2001			 # maximal measurable distance
        self._max_data = 20              # maximum number of datapoints per angle
        self._out_of_range = 2001     # indicates invalid data
        ...
        self._result = np.array([self._out_of_range for _ in range(D_ANGLE)], dtype=np.int32)#different point
    ...
    def _scan(self):
        ...
        while self._is_scanning:
            ...
            for angle in range(D_start_angle, D_end_angle):
                if distances_pnt[angle] == 0:
                    self._result[angle-D_start_angle] = self._out_of_range
                else:
                    self._result[angle-D_start_angle] = self._distances[angle][:distances_pnt[angle]].mean()
