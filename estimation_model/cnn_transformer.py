...
from tensorflow.keras.layers import Conv1D, Input,BatchNormalization, MaxPooling1D, LSTM, Dense, Dropout, TimeDistributed
...
class CNNModel:
    ...
    def build_model(self, input_shape, output_shape):
        ...
        # Reshape if input is (sequence_length,)
        ...
        # Transformer Block 이후 적용
        # CNN 출력의 시퀀스 길이와 특징 수 확인
        ...
        position_embedding = tf.keras.layers.Embedding(input_dim=sequence_length, output_dim=feature_dim)(position)
        x = x + position_embedding  # Positional encoding 추가
        
        # Transformer Layer
        transformer_layer = MultiHeadAttention(num_heads=2, key_dim=64)
        attention_output = transformer_layer(x, x)
        ...
        # 추가적인 Transformer Block을 원한다면 여기에 추가 가능
...
if __name__=="__main__":
    ...
    # 모델 학습
    rnn_model.train_and_evaluate_model(X_train, y_train, epochs=20,batch_size=128, model_name='cnn_transformer.h5')

    # 모델을 사용해 최종 예측 및 평가
    rnn_model.predict_and_evaluate(X_train, y_train, model_path='cnn_transformer.h5', label_mapping=label_mapping)