...
if __name__=="__main__":
    ...
    import time
    def single_inference_with_delay(model, X_data, interval=0.05):
        delays = []
        for i in range(len(X_data)):
            # Start time
            start_time = time.time()

            # Model prediction for the current data point
            _ = model.predict(X_data[i:i+1])

            # End time
            end_time = time.time()
            
            # Calculate delay and store it
            delay = end_time - start_time
            delays.append(delay)
            
            # Print delay
            print(f"Inference {i+1}/{len(X_data)} delay: {delay:.4f} seconds")
        ...
        return delays
    # 모델을 사용해 최종 예측 및 평가
    delays = single_inference_with_delay(rnn_model.model, X_test, interval=0.05)
