...
class CNNModel:
    ...
    def build_model(self, input_shape, output_shape):
        ...
        # 입력 데이터를 2차원에서 3차원으로 변환 (Transformer 레이어는 3차원 데이터를 기대)
        x = tf.expand_dims(inputs, axis=1)  # (batch_size, 1, input_dim)
        
        # 포지셔널 인코딩 추가
        positional_encoding = PositionalEncoding(sequence_length=1, d_model=input_shape[0])
        x = positional_encoding(x)  # (batch_size, 1, input_dim + positional_encoding)
        
        # Transformer (MultiHeadAttention) 레이어 추가
        attention_output = MultiHeadAttention(num_heads=4, key_dim=64)(x, x)
        attention_output = LayerNormalization()(attention_output + x)
        
        # Dense 레이어
        x = Flatten()(attention_output)  # Transformer 출력 flattened
...
if __name__=="__main__":
    ...
    # 모델 학습
    #rnn_model.train_and_evaluate_model(X_train, y_train, epochs=100,batch_size=128, model_name='cnn_transfromer_static_mun300_to_mun100.h5')

    # 모델을 사용해 최종 예측 및 평가
    rnn_model.predict_and_evaluate(X_test, y_test, model_path='cnn_transfromer_static_mun300_to_mun100.h5', label_mapping=label_mapping)