...
class MotionDataset(Dataset):
    ...
    def load_csv_file(self, file, global_idx_offset):
        ...
        # 라벨 필터링을 제거하고 모든 데이터를 사용합니다.
        # labels = labels[labels != 5]  # 라벨 5 제거하지 않음
        # labels = np.where(labels > 5, labels - 1, labels)  # 라벨을 재매핑하지 않음
...
if __name__=="__main__":
    ...
    test_set = MotionDataset(root_directory, folder_names=usr_name[0])
